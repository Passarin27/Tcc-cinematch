...
async function verificarFavorito() {
  ...
  const jaFavorito = filmes.find(f => String(f.tmdb_id) === String(filmeId));
...
async function toggleFavorito() {
  ...
  const filmesResp = await fetch(`${API_URL}/filmes`, {
    headers: { Authorization: `Bearer ${token}` }
  });
  const filmes = await filmesResp.json();

  let filme = filmes.find(f => String(f.tmdb_id) === String(filmeId));
