...
async function verificarFavorito() {
    const res = await fetch("http://localhost:3333/filmes", {
...
async function obterListaFavoritos() {
    ...
    if (!favoritos) {
        favoritos = await fetch("http://localhost:3333/listas", {
            method: "POST",
            headers: {
...
async function toggleFavorito() {
    ...
    // busca filmes salvos
    const filmesResp = await fetch("http://localhost:3333/filmes", {
        headers: { Authorization: `Bearer ${token}` }
    });
    ...
    if (coracao.classList.contains("ativo")) {
        ...
        await fetch(`http://localhost:3333/listas/${listaId}/filmes/${filme.id}`, {
            method: "DELETE",
            headers: { Authorization: `Bearer ${token}` }
        });
    ...
    if (!filme) {
        filme = await fetch("http://localhost:3333/filmes", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`
    ...
    await fetch(`http://localhost:3333/listas/${listaId}/filmes`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`
