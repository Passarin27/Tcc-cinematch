...
async function mostrarRecomendacoes() {
  ...
  while (filmesVisiveis < 10) {
    ...
    for (const filme of data.results) {
      ...
      card.querySelector(".coracao").addEventListener("click", () => {
        favoritar(
          card.querySelector(".coracao"),
          filme.id,
          filme.title,
          filme.poster_path
        );
...
/* =========================
   FAVORITAR
========================= */
async function favoritar(el, tmdbId, titulo, posterPath) {
  ...
  if (filmesVisiveis < 10) mostrarRecomendacoes();
}
...
async function mostrarListas() {
  ...
  for (const lista of listas) {
    ...
    for (const filme of filmes) {
      ...
      const ano = detalhes.release_date?.split("-")[0] || "—";
      ...
      card.innerHTML = `
        ...
        <span class="coracao ativo">❤</span>
      `;

      card.querySelector(".coracao").addEventListener("click", () => {
        removerFavorito(card, lista.id, filme.id);
      });
...
/* =========================
   REMOVER FAVORITO
========================= */
async function removerFavorito(card, listaId, filmeId) {
